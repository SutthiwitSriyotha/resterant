  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-gray-50 min-h-screen font-sans">
      ...
      <div className="mb-8 flex flex-wrap gap-4">
        <Link
          href="/dashboard/menu"
          className="flex-1 min-w-[140px] px-4 py-2 sm:px-5 sm:py-3 bg-blue-600 text-white rounded-xl shadow-md hover:bg-blue-700 transition text-center font-medium text-sm sm:text-base"
        >
          จัดการเมนูอาหาร
        </Link>
        <Link
          href="/dashboard/orders"
          className="flex-1 min-w-[140px] px-4 py-2 sm:px-5 sm:py-3 bg-green-600 text-white rounded-xl shadow-md hover:bg-green-700 transition text-center font-medium text-sm sm:text-base"
        >
          จัดการออร์เดอร์
        </Link>
        {storeId && (
          <Link
            href={`/order/${storeId}`}
            className="flex-1 min-w-[140px] px-4 py-2 sm:px-5 sm:py-3 bg-purple-600 text-white rounded-xl shadow-md hover:bg-purple-700 transition text-center font-medium text-sm sm:text-base"
          >
            หน้าออเดอร์ลูกค้า
          </Link>
        )}
        ...
        {tableSaved ? (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition text-sm sm:text-base font-medium"
            >
              แก้ไข
            </button>
          </div>
          ...
          <div className="flex flex-col gap-4 text-sm sm:text-base">
            ...
            <button
              ...
              className="px-4 py-2 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700 transition text-sm sm:text-base font-medium"
            >
      ...
      {loading ? (
        <p className="text-gray-800 text-sm sm:text-base">กำลังโหลดเมนู...</p>
      ) : menus.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {menus.map((menu) => (
            ...
            >
              ...
              <p className="text-gray-800 font-medium text-sm sm:text-sm mt-1">
                ราคา: {menu.price} บาท
              </p>
              {menu.description && (
                <p className="text-gray-700 text-sm sm:text-sm mt-1">{menu.description}</p>
              )}
              {menu.addOns && menu.addOns.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {menu.addOns.map((a) => (
                    <span
                      key={a.id}
                      className="text-gray-900 sm:text-sm bg-gray-200 px-2 py-1 rounded-full font-medium"
                    >
                      {a.name} +{a.price} บาท
                    </span>
                  ))}
